...
<template>
  ...
  >
    <div
      v-if="collapsed"
      class="relative md:flex h-full flex-col items-start pl-4 pt-1 pb-3 titlebar-no-drag"
    >
      <div class="flex flex-col items-start gap-0.5">
        ...
        >
          <button
            class="collapsed-sidebar-btn group relative flex h-[34px] w-[34px] items-center justify-center titlebar-no-drag rounded-[10px]"
            :class="[
          ...
          >
            <component :is="section.icon" class="w-[18px] h-[18px]" />
            <span
      ...
      <div
        ...
        class="absolute left-full top-0 z-50 ml-2 w-[280px] max-h-[calc(100vh-1rem)] overflow-y-auto rounded-[14px] border border-border bg-popover p-4 shadow-lg"
      ...
      >
        <div class="mb-3 flex items-center justify-between gap-3 border-b border-border/70 pb-3">
          <div class="flex min-w-0 items-center gap-2">
            ...
            />
            <div class="min-w-0">
              <h3 class="truncate text-[13px] font-semibold">{{ activeCollapsedMeta?.label }}</h3>
          ...
          <button
            class="collapsed-sidebar-btn p-1 text-muted-foreground hover:text-foreground rounded-[8px]"
    ...
    <template v-if="!collapsed">
      ...
      <!-- Scrollable Content -->
      <div class="config-panel-scroll flex-1 overflow-y-auto overflow-x-hidden px-5 py-4 pb-8 md:px-5 md:py-4 md:pb-4 space-y-6">
        <!-- Server / CLI Toggle (always visible) -->
        <section>
          <div class="space-y-3">
            <div class="flex p-1 metal-surface rounded-[12px]">
              <button
                @click="setBackendMode('server')"
                class="flex-1 h-[40px] text-[13px] font-medium rounded-[10px] flex items-center justify-center gap-2 transition-colors"
                :class="
                  config.backendMode === 'server'
                    ? 'bg-background text-foreground shadow-sm'
                    : 'text-muted-foreground hover:text-foreground'
            ...
            <div v-if="config.backendMode === 'server'" class="flex gap-3">
              <button
                @click="startServer"
                :disabled="sdServerRunning || isBooting || !backendValid"
                class="flex-1 h-[36px] text-[13px] font-medium rounded-[10px] flex items-center justify-center gap-1.5 metal-surface transition-colors"
                :class="
                  sdServerRunning || isBooting || !backendValid
                    ? 'text-muted-foreground'
                    : 'text-green-600'
        ...
        <section>
          <button
            ...
            class="w-full flex items-center justify-between text-[13px] font-semibold text-foreground h-[38px]"
          >
            <span>Presets</span>
            <div class="flex items-center gap-2">
              <span class="text-[11px] text-muted-foreground">{{ presets.length }}</span>
          ...
          <div v-show="expandedSections.presets" class="space-y-3 pt-1">
            <div class="flex items-center gap-3">
              ...
              <button
                @click="deleteSelectedPreset"
                :disabled="!selectedPreset || selectedPreset.builtin"
                class="h-[36px] w-[36px] metal-icon-button flex items-center justify-center rounded-[10px] text-muted-foreground hover:text-destructive transition-colors disabled:opacity-30"
                title="Delete preset"
              >
            ...
            <div class="flex items-center gap-3">
              <input
                ...
                type="text"
                placeholder="Save as..."
                class="h-[36px] flex-1 min-w-0 text-[13px] rounded-[10px]"
                @keyup.enter="saveCurrentPreset"
              />
              <button
                @click="saveCurrentPreset"
                :disabled="!presetName.trim()"
                class="h-[36px] px-4 text-[13px] font-medium rounded-[10px] primary-metal-button text-white disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Save
              </button>
            </div>
        ...
        <section>
          ...
          <div class="flex p-1 metal-surface rounded-[12px] mb-3">
            <button
              @click="setLoadMode('standard')"
              class="flex-1 h-[40px] text-[13px] font-medium rounded-[10px] transition-colors"
              :class="
                config.loadMode === 'standard'
                  ? 'bg-background text-foreground shadow-sm'
                  : 'text-muted-foreground'
          ...
          <div class="space-y-3">
            ...
            <div v-if="config.loadMode === 'standard'" class="space-y-2">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Model Checkpoint</label>
                <Select
                  v-model="config.standardModel"
                  size="md"
            ...
            <div v-else class="space-y-2">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Diffusion Model</label>
                <Select
                  v-model="config.diffusionModel"
                  size="md"
                  placeholder="Select..."
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">High Noise</label>
                  <Select
                    v-model="config.highNoiseDiffusionModel"
                    size="md"
                    placeholder="None"
                ...
                <div>
                  <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Uncond Diffusion</label>
                  <Select
                    v-model="config.uncondDiffusionModel"
                    size="md"
                    placeholder="None"
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">T5XXL</label>
                  <Select
                    v-model="config.t5xxlModel"
                    size="md"
                    placeholder="None"
                ...
                <div>
                  <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">LLM</label>
                  <Select
                    v-model="config.llmModel"
                    size="md"
                    placeholder="None"
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">CLIP-L</label>
                  <Select
                    v-model="config.clipModel"
                    size="md"
                    placeholder="None"
                ...
                <div>
                  <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">CLIP-G</label>
                  <Select
                    v-model="config.clipGModel"
                    size="md"
                    placeholder="None"
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">CLIP Vision</label>
                <Select
                  v-model="config.clipVisionModel"
                  size="md"
                  placeholder="None"
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">LLM Vision</label>
                  <Select
                    v-model="config.llmVisionModel"
                    size="md"
                    placeholder="None"
                ...
                <div>
                  <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Emb. Connectors</label>
                  <Select
                    v-model="config.embeddingsConnectorsModel"
                    size="md"
                    placeholder="None"
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">VAE</label>
                <Select
                  v-model="config.vaeModel"
                  size="md"
                  placeholder="None"
                ...
                <p
                  v-if="
                    ...
                    !(config.diffusionModel || config.standardModel || '')
                      ...
                      .includes('flux')
                  "
                  class="text-[11px] text-yellow-600 mt-[6px] leading-relaxed font-medium"
                >
                  ae.sft is for Flux. Might fail with SDXL/Pony.
                </p>
              </div>
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">VAE Tile</label>
                <input
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Audio VAE</label>
                <Select
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">VAE Format</label>
                <Select
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">ControlNet</label>
                <Select
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">PhotoMaker</label>
                <Select
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Upscale</label>
                <Select
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">TAESD</label>
                <Select
        ...
        <section>
          ...
          >
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Steps</label>
                <input
                  v-model.number="config.steps"
                  type="number"
                  min="1"
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">CFG Scale</label>
                <input
                  v-model.number="config.cfgScale"
                  type="number"
                  step="0.5"
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Guidance</label>
                <input
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Clip Skip</label>
                <input
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Seed (-1 random)</label>
              <input
                v-model.number="config.seed"
                type="number"
                class="w-full"
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Flow Shift</label>
                <input
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">ETA</label>
                <input
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">SLG Scale</label>
                <input
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Img CFG</label>
                <input
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">SLG Start</label>
                <input
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">SLG End</label>
                <input
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Skip Layers</label>
              <input
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Sigmas</label>
              <input
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Extra Sample Args</label>
              <input
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Extra Tiling Args</label>
              <input
        ...
        <section>
          ...
          >
            LoRA Modules
            <div class="flex items-center gap-2">
              <span class="text-[11px] bg-muted px-2 py-0.5 rounded-lg font-medium">{{ config.loras.length }}</span>
          ...
          >
            <div
              ...
              :key="index"
              class="flex items-center gap-3 p-2 rounded-[10px] bg-card"
            >
              <div class="flex-1 min-w-0">
                <label class="text-[11px] font-medium text-muted-foreground uppercase tracking-wide block mb-[6px]">LoRA {{ index + 1 }}</label>
                <Select
                  v-model="lora.path"
                  size="md"
              ...
              <div class="shrink-0">
                <label class="text-[11px] font-medium text-muted-foreground uppercase tracking-wide block mb-[6px] text-center">Strength</label>
                <input
                  ...
                  min="0"
                  max="2"
                  class="w-20 h-[36px] text-center font-medium rounded-[10px]"
                  title="Strength"
                />
              </div>
              <button
                @click="configStore.removeLora(index)"
                class="self-end p-2 metal-icon-button text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors rounded-[10px]"
                title="Remove LoRA"
              >
                <Trash2 class="w-3.5 h-3.5" />
              </button>
            ...
            <button
              @click="addNewLora"
              class="w-full h-[36px] text-[13px] font-medium flex items-center justify-center gap-1.5 rounded-[10px] metal-surface hover:text-foreground transition-colors"
            >
        ...
        <section>
          ...
          >
            Embeddings
            <div class="flex items-center gap-2">
              <span class="text-[11px] bg-muted px-2 py-0.5 rounded-lg font-medium">{{
          ...
          >
            ...
            >
              <div class="flex-1 flex items-center gap-3 overflow-hidden">
                <span
                  class="text-[11px] uppercase font-medium text-muted-foreground px-2 py-1 bg-muted rounded-lg shrink-0"
                  >TI</span
                >
                <Select
                  :model-value="emb"
              ...
              <button
                ...
                class="p-2 metal-icon-button text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors rounded-[10px]"
        ...
        <section>
          ...
          >
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Scheduler</label>
                <Select
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Sampler</label>
                <Select
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">RNG</label>
                <Select
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Sampler RNG</label>
                <Select
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Cache Mode</label>
                <Select
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">SCM Policy</label>
                <Select
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Cache Options</label>
              <input
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">SCM Mask</label>
              <input
        ...
        <section>
          ...
          >
            <div class="grid grid-cols-2 gap-3">
              <label class="flex items-center gap-2 text-[13px] font-medium cursor-pointer">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Runtime Backend</label>
                <input
                  v-model="config.backendAssignment"
                  type="text"
                  placeholder="auto, cpu, cuda0"
                  class="w-full"
                />
              </div>
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Params Backend</label>
                <input
                  v-model="config.paramsBackendAssignment"
                  type="text"
                  placeholder="cpu or diffusion=gpu"
                  class="w-full"
                />
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Threads</label>
                <input
              ...
              <div>
                <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Max VRAM GiB</label>
                <input
                  v-model.number="config.maxVram"
                  type="number"
                  step="0.1"
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Quantization</label>
              <Select
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Tensor Type Rules</label>
              <input
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Prediction</label>
              <Select
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">LoRA Apply Mode</label>
              <Select
            ...
            <div>
              <label class="text-[12px] text-muted-foreground font-medium block mb-[6px]">Chroma T5 Mask Pad</label>
              <input
        ...
        <section v-if="config.backendMode === 'server'">
          <h3 class="text-[13px] font-semibold text-foreground mb-2">Logs</h3>
          <div
            class="h-32 bg-muted/20 rounded-[10px] p-2 overflow-y-auto text-[11px] font-mono text-foreground leading-tight"
          >
