...
export function fetchJson<T>(url: string): Promise<T> {
  return new Promise((resolve, reject) => {
    https
      .get(url, { headers: { 'User-Agent': 'StableUI' } }, (response) => {
        if (
...
export function downloadFile(
  ...
  options: {
    ...
    registerResume?: (resume: () => void) => void
    append?: boolean
  } = {}
): Promise<string> {
  return new Promise((resolve, reject) => {
    let settled = false
    let activeRequest: ReturnType<typeof https.get> | ReturnType<typeof http.get> | null = null
    let paused = false
    ...
    options.registerResume?.(() => {
      if (!paused || settled) return
      paused = false
      doDownload(url)
    })

    const resolveLocation = (loc: string, baseUrl: string): string => {
    ...
    const doDownload = (downloadUrl: string): void => {
      ...
      if (options.append && fs.existsSync(destPath)) {
      ...
      console.log(`[Download] GET ${normalized}`)

      activeRequest = transport
        .get(normalized, { headers, timeout: 30000 }, (response) => {
          const code = response.statusCode
          console.log(`[Download] Response ${code} for ${normalized}`)

          if ((code === 301 || code === 302 || code === 307 || code === 308) && response.headers.location) {
            response.resume()
            const next = resolveLocation(response.headers.location, normalized)
            console.log(`[Download] Redirect ${code} -> ${next}`)
            doDownload(next)
            return
          }
          ...
          const total = contentRange
            ...
            : contentLength
              ? contentLength + (options.append && fs.existsSync(destPath) ? fs.statSync(destPath).size : 0)
              : null

          console.log(`[Download] Content-Length: ${contentLength}, Total: ${total}`)

          const isAppend = code === 206
          const file = fs.createWriteStream(destPath, { flags: isAppend ? 'a' : 'w' })
          ...
          pipeline(response, progressStream, file)
            .then(() => {
              if (settled || paused) return
              settled = true
              activeRequest = null
              console.log(`[Download] Complete: ${destPath} (${received} bytes)`)
              resolve(destPath)
            })
            .catch((err) => {
              if (!settled) {
