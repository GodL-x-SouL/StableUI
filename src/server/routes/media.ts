...
function galleryItem(ctx: AppContext, filename: string): Record<string, unknown> {
  const filePath = path.join(ctx.paths.outputDir, filename)
  const stat = fs.statSync(filePath)
  const params = parseParams(readPngParams(filePath))
  return {
    filename,
    size: stat.size,
    ...
    thumbnail: `/output/${encodeURIComponent(filename)}`,
    type: path.extname(filename).slice(1).toLowerCase(),
    metadata: params
  }
}

function readHistory(ctx: AppContext): unknown[] {
  try {
    if (!fs.existsSync(ctx.paths.historyFile)) return []
    const parsed = JSON.parse(fs.readFileSync(ctx.paths.historyFile, 'utf8'))
...
export function registerMediaRoutes(app: Express, ctx: AppContext): void {
  app.get(['/api/gallery', '/gallery'], (_req, res) => {
    try {
  ...
  app.get(['/api/gallery/index', '/gallery/index'], (_req, res) => {
    try {
      res.json({ items: listGallery(ctx).map((filename) => galleryItem(ctx, filename)) })
    } catch (error: any) {
  ...
  app.get(['/api/history', '/history'], (_req, res) => {
    res.json({ history: readHistory(ctx) })
  })
  ...
  app.get('/api/open-folder', (req, res) => {
    ...
    const folderMap: Record<string, string> = {
      ...
      output: ctx.paths.outputDir,
      custom: ctx.paths.customDir
    }
    const folderPath = folderMap[type]
    if (!folderPath) return res.status(400).json({ success: false, error: 'Invalid folder type' })
    fs.mkdirSync(folderPath, { recursive: true })
    const absPath = path.resolve(folderPath)
    const command = process.platform === 'darwin' ? `open "${absPath}"` : process.platform === 'win32' ? `start "" "${absPath}"` : `xdg-open "${absPath}"`
    exec(command, (error) => {
      if (error) {
        console.warn('[OpenFolder] Failed to open folder:', error.message)
      }
    })
    res.redirect('/')
  })
