...
export function registerBackendRoutes(app: Express, ctx: AppContext): void {
  ...
  app.get('/api/paths', (_req, res) => {
    res.json({
      backendDir: ctx.paths.backendDir,
      modelsDir: ctx.paths.modelsDir,
      outputDir: ctx.paths.outputDir,
      configured: {
        backendDir: !!ctx.state.backendConfig.backendDir,
        modelsDir: !!ctx.state.backendConfig.modelsDir,
        outputDir: !!ctx.state.backendConfig.outputDir
      }
    })
  })
  ...
  app.post('/api/paths', (req, res) => {
    ...
    if (type !== 'modelsDir' && type !== 'outputDir' && type !== 'backendDir') return res.status(400).json({ error: 'type must be modelsDir, outputDir, or backendDir' })
    if (!newPath || typeof newPath !== 'string') return res.status(400).json({ error: 'path is required' })
    ;(ctx.paths as any)[type] = newPath
    ctx.state.backendConfig[type] = newPath
    ctx.saveBackendConfig()
    res.json({ success: true, [type]: newPath })
  })
}
